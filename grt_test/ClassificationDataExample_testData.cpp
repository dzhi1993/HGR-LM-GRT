...
int main (int argc, const char * argv[])
{
	...
	for (UINT k = 0; k < 10; ++k) {// For the number of gestures
		...
		for (UINT x = 0; x < 10; ++x) {// Generate 10 samples for current gesture.
			...
			while (framesPerSample--) {	
				...
				sample[5] = Thumb.tipPosition().distanceTo(Index.tipPosition());
				sample[6] = Index.tipPosition().distanceTo(Middle.tipPosition());
				sample[7] = Middle.tipPosition().distanceTo(Ring.tipPosition());
				sample[8] = Ring.tipPosition().distanceTo(Pinky.tipPosition());
	...
	if( !trainingData.save( "testData_static_10numbers.grt" ) ){
		std::cout << "ERROR: Failed to save dataset to file!\n";
		return EXIT_FAILURE;
	}
	
	//This can then be loaded later
	if( !trainingData.load( "testData_static_10numbers.grt" ) ){
	...
	if( !trainingData.save( "testData_static_10numbers.csv" ) ){
	...
	if( !trainingData.load( "testData_static_10numbers.csv" ) ){
